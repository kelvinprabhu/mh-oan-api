...
async def stream_chat_messages(
    ...
    # Using the context manager manually as before, but without the extra indentation level from propagate_attributes
    try:
        response_stream = await stream_context.__aenter__()
    except Exception as e:
        logger.error(f"Error entering stream context: {str(e)}")
        raise
